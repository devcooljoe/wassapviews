...
class HomeScreen extends StatelessWidget {
  ...
  void _showCustomDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) => AlertDialog(
        ...
        content: const Text(
            'Please join our official Telegram channel for more info, questions, updates, tutorial, etc.. .'),
        actions: <Widget>[
          CustomTextButton(
            text: 'Join Us',
            onPressed: () async {
  ...
  void browserDownloadVCF(BuildContext context, String date) async {
    LoadingDialogController _controller =
        context.read(loadingDialogProvider.notifier);
    try {
      ...
      if (_response.statusCode == 200 || _response.statusCode == 201) {
        ...
        if (_fetchedData['status'] == 'success') {
          ...
          showDialog(
            ...
            builder: (BuildContext context) => AlertDialog(
              ...
              content: const Text(
                  'VCF file has been requested completely. Click the \'Download VCF\' button to open your browser and download the VCF file.'),
              actions: <Widget>[
                CustomTextButton(
                  text: 'Download VCF',
                  onPressed: () async {
          ...
          showDialog(
            ...
            builder: (BuildContext context) => AlertDialog(
              ...
              content: Text(UserSharedPreferences.getShared() == 'true'
                  ? 'Come back later for more new VCF files.'
                  : 'Click the \'SHARE NOW\' button to share our app link on your WhatsApp status and in 5 groups.'),
              actions: <Widget>[
                UserSharedPreferences.getShared() == 'true'
                    ? SizedBox.shrink()
                    : CustomTextButton(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'An unexpected error occured. StatusCode: ${_response.statusCode}'),
            duration: const Duration(seconds: 4),
          ),
        );
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'Storage permission not granted by device. Please use another device.'),
          duration: Duration(seconds: 4),
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('An error occured! Could not get the downloads directory.'),
          duration: Duration(seconds: 4),
        ),
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    void setOnboardPage(BuildContext context) {
      ...
      if (pageController.hasClients) {
        pageController.animateToPage(initpage,
    ...
    PremiumPlanStatusController _premiumPlanStatusController =
        context.read(premiumPlanStatusProvider.notifier);
    ShareAppController _shareAppController =
        context.read(shareAppProvider.notifier);
    if (_premiumPlanStatusController.getPremiumPlanStatus() != 'active') {
      Future.delayed(Duration(milliseconds: 1000),
          () => GlobalVariables.loadInterstitialAd());
      Timer.periodic(Duration(seconds: 60),
          (Timer t) => GlobalVariables.loadInterstitialAd());
    }
    if (appJustLoaded) {
      Future.delayed(Duration(seconds: 1), () => fetchContactCount(context));
      Future.delayed(
          Duration(seconds: 1), () => checkPremiumPlanStatus(context));
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: <Widget>[
                                ...
                                children: <Widget>[
                                  ...
                                  SmoothPageIndicator(
                                    ...
                                    effect: WormEffect(
                                      dotHeight: 5,
                                      dotWidth: 5,
                                      dotColor: GlobalVariables.isDarkMode()
                                          ? AppColors.grey4
                                          : AppColors.grey2,
                                      activeDotColor:
                            ...
                            ListView.builder(
                              ...
                              itemBuilder: (context, int) {
                                ...
                                return Material(
                                  ...
                                  child: InkWell(
                                    onTap: () async {
                                      PremiumPlanStatusController
                                          _premiumPlanStatusController =
                                          context.read(premiumPlanStatusProvider
                                              .notifier);
                                      if (_premiumPlanStatusController
                                              .getPremiumPlanStatus() !=
                                          'active') {
                                        showDialog(
                                            context: context,
                                            builder: (BuildContext context) {
                                              return AlertDialog(
                                                ...
                                                content: Text(
                                                    'Join our premium plan to access this file.'),
                                                actions: [
                                                  CustomTextButton(
                                                      text: 'OK',
                                                      onPressed: () {
          ...
          Consumer(builder: (context, watch, widget) {
            return (!(watch(shareAppProvider) as bool) &&
                    (UserSharedPreferences.getUserPhoneNumber() != '') &&
                    (_premiumPlanStatusController.getPremiumPlanStatus() !=
                        'active'))
                ? Container(
                    ...
                    child: AlertDialog(
                      ...
                      content: Text(
                          'Please click the \'SHARE NOW\' button to share our app link on your WhatsApp status in order to use this app.'),
                      actions: <Widget>[
                        CustomTextButton(
                          text: 'SHARE NOW',
                          onPressed: () async {
          ...
          Consumer(builder: (context, watch, widget) {
            return (watch(internetProvider) as bool)
                ? SizedBox.shrink()
                : InternetDialog();
          }),
          Consumer(builder: (context, watch, widget) {
            return (watch(loadingDialogProvider) as bool)
                ? LoadingDialog()
                : SizedBox.shrink();
          }),
        ],
  ...
  try {
    ...
    if (_response.statusCode == 200 || _response.statusCode == 201) {
      ...
      if (_fetchedData['status'] == 'success') {
        ...
        ContactCountController _controller =
            context.read(contactCountProvider.notifier);
...
void checkPremiumPlanStatus(BuildContext context) async {
  PhoneNumberController _phoneNumberController =
      context.read(phoneNumberProvider.notifier);
  PremiumPlanController _premiumPlanController =
      context.read(premiumPlanProvider.notifier);
  EndPlanDateController _endPlanDateController =
      context.read(endPlanDateProvider.notifier);
  PremiumPlanStatusController _premiumPlanStatusController =
      context.read(premiumPlanStatusProvider.notifier);
  try {
    final _response = await post(
    ...
    if (_response.statusCode == 200 || _response.statusCode == 201) {
      ...
      if (_fetchedData['status'] == 'success') {
        _premiumPlanStatusController
            .setPremiumPlanStatus(_fetchedData['data']['status']);
